	...
	for (int i = 0; i < NUMBER_OF_LIGHTS_SUPPORTED; i++) {
		...
		if (light[i].position.w != zero_f) { // point light source
			...
			if (light[i].spot_cutoff_angle < 180.0) { // [0.0f, 90.0f] or 180.0f
                ...
                tmp_float2 = dot(-L_eye, tmp_li1);
                tmp_float3 = dot(-L_eye, tmp_li2);
				if (tmp_float >= cos(radians(light[i].spot_cutoff_angle))) {
                    if(i==0) tmp_float = pow(tmp_float, light[i].spot_exponent);
                    if(i==1) tmp_float = cos(num_f*acos(tmp_float)*0.25);
                    if(i==2){
                        tmp_float2 = cos(num_f*acos(tmp_float2)*0.1);
                        tmp_float3 = cos(num_f*acos(tmp_float3)*0.1);
