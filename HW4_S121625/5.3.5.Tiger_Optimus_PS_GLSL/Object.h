...
Material_Parameters material_cir;
...
void prepare_cir(void) { // Draw circle.
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(cir), &cir[0][0], GL_STATIC_DRAW);
	...
	glVertexAttribPointer(LOC_NORMAL, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), BUFFER_OFFSET(3 * sizeof(float)));
	glEnableVertexAttribArray(1);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	glBindVertexArray(0);
	material_cir.ambient_color[0] = 0.03f;
	material_cir.ambient_color[1] = 0.03f;
	material_cir.ambient_color[2] = 0.03f;
	material_cir.ambient_color[3] = 1.0f;

	material_cir.diffuse_color[0] = 0.5f;
	material_cir.diffuse_color[1] = 0.1f;
	material_cir.diffuse_color[2] = 0.1f;
	material_cir.diffuse_color[3] = 1.0f;

	material_cir.specular_color[0] = 0.5f;
	material_cir.specular_color[1] = 0.1f;
	material_cir.specular_color[2] = 0.1f;
	material_cir.specular_color[3] = 1.0f;

	material_cir.specular_exponent = 20.5f;

	material_cir.emissive_color[0] = 0.0f;
	material_cir.emissive_color[1] = 0.0f;
	material_cir.emissive_color[2] = 0.0f;
	material_cir.emissive_color[3] = 1.0f;
}

void set_material_cir(void) {
	// assume ShaderProgram_PS is used
	glUniform4fv(loc_material.ambient_color, 1, material_cir.ambient_color);
	glUniform4fv(loc_material.diffuse_color, 1, material_cir.diffuse_color);
	glUniform4fv(loc_material.specular_color, 1, material_cir.specular_color);
	glUniform1f(loc_material.specular_exponent, material_cir.specular_exponent);
	glUniform4fv(loc_material.emissive_color, 1, material_cir.emissive_color);
}

void draw_cir(void) {
	glFrontFace(GL_CCW);

	glBindVertexArray(VBO_cir);
	for (int i = 0; i < 21; i++) {
		for (int j = 0; j < 40; j++) {
...
//	for (int i = 0; i < 840; i ++){
	//	glDrawArrays(GL_POINT_BIT, i, 1);
	
	glBindVertexArray(0);
}


GLuint window_VBO, window_VAO;
GLfloat window_vertices1[6][3] = {
	{ 0, 250.0f,250.0f } ,{ 0.0f, 0.0f, 250.0f } , { 0.0f, 250.0f, 0.0f },
	{ 0, 0,0 } ,{ 0.0f, 0.0f, 250.0f } ,{ 0.0f, 250.0f, 0.0f }
};
Material_Parameters material_window;
void prepare_window(void) { // draw coordinate axes
						  // initialize vertex buffer object
	glGenBuffers(1, &window_VBO);

	glBindBuffer(GL_ARRAY_BUFFER, window_VBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(window_vertices1), &window_vertices1[0][0], GL_STATIC_DRAW);
	...
	glEnableVertexAttribArray(0);
	 glVertexAttribPointer(LOC_NORMAL, 3, GL_FLOAT, GL_FALSE,2* sizeof(float), BUFFER_OFFSET(0* sizeof(float)));
	glEnableVertexAttribArray(1);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
	glBindVertexArray(0);
	material_window.ambient_color[0] = 0.03f;
	material_window.ambient_color[1] = 0.03f;
	material_window.ambient_color[2] = 0.03f;
	material_window.ambient_color[3] = 1.0f;

	material_window.diffuse_color[0] = 0.5f;
	material_window.diffuse_color[1] = 0.5f;
	material_window.diffuse_color[2] = 0.5f;
	material_window.diffuse_color[3] = 1.0f;

	material_window.specular_color[0] = 0.5f;
	material_window.specular_color[1] = 0.5f;
	material_window.specular_color[2] = 0.5f;
	material_window.specular_color[3] = 1.0f;

	material_window.specular_exponent = 20.5f;

	material_window.emissive_color[0] = 0.0f;
	material_window.emissive_color[1] = 0.0f;
	material_window.emissive_color[2] = 0.0f;
	material_window.emissive_color[3] = 1.0f;
}
void set_material_window(void) {

	glUniform4fv(loc_material.ambient_color, 1, material_window.ambient_color);
	glUniform4fv(loc_material.diffuse_color, 1, material_window.diffuse_color);
	glUniform4fv(loc_material.specular_color, 1, material_window.specular_color);
	glUniform1f(loc_material.specular_exponent, material_window.specular_exponent);
	glUniform4fv(loc_material.emissive_color, 1, material_window.emissive_color);
}
...
void prepare_floor(void) { // Draw coordinate axes.
	...
	material_floor.ambient_color[0] = 0.03f;
	material_floor.ambient_color[1] = 0.03f;
	material_floor.ambient_color[2] = 0.03f;
	...
	material_floor.specular_color[0] = 0.1f;
	material_floor.specular_color[1] = 0.1f;
